...
type PageId = 'overview' | 'trend' | 'official2026' | 'operations' | 'context' | 'priority' | 'report' | 'sources'
...
function LineChart({ rows, label, showLabels = false }: { rows: Array<{ month: string; count: number }>; label: string; showLabels?: boolean }) {
  ...
  const ticks = [0, .25, .5, .75, 1].map((ratio) => Math.round(min + range * ratio))
  return <div className="integrated-chart"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={label}>{ticks.map((tick) => <g className="axis-tick" key={tick}><line x1={pad} x2={width - pad} y1={y(tick)} y2={y(tick)} /><text x={pad - 7} y={y(tick) + 4} textAnchor="end">{number(tick)}</text></g>)}<path d={path} />{rows.map((row, i) => { const peak = row.count === max; return <g className={peak ? 'peak' : ''} key={`${row.month}-${i}`}><circle cx={x(i)} cy={y(row.count)} r={peak ? 6 : 4}><title>{row.month} · {number(row.count)}건</title></circle>{showLabels ? <text className="point-label" x={x(i)} y={Math.max(y(row.count) - 11, 12)} textAnchor="middle">{number(row.count)}</text> : null}<text className="month-label" x={x(i)} y={height - 10} textAnchor="middle">{row.month.slice(2)}</text></g>})}</svg></div>
}
...
export default function UnifiedApp() {
  ...
  const selectedCheck = data.officialComplaints.latestChecks.find((row) => row.issue === selected.issue) ?? data.officialComplaints.latestChecks[0]
  const currentMonthly = official.monthly.map((row) => ({ month: month(String(row.crtr_ym)), count: Number(row.tot_nocs) }))
  const trendSource = trendDistrict === '전체' ? data.baseline.monthly : data.baseline.districts.find((row) => row.district === trendDistrict)?.monthlyComplaints ?? []
  const historicalMonthly = trendSource.filter((row) => trendYear === '전체' || row.month.startsWith(trendYear)).map((row) => ({ month: row.month, count: row.count }))
  const trendTotal = historicalMonthly.reduce((sum, row) => sum + row.count, 0)
  const trendAverage = historicalMonthly.length ? Math.round(trendTotal / historicalMonthly.length) : 0
  const trendPeak = historicalMonthly.reduce((peak, row) => row.count > peak.count ? row : peak, historicalMonthly[0] ?? { month: '-', count: 0 })
  const trendChange = historicalMonthly.length > 1 && historicalMonthly[0].count ? (historicalMonthly.at(-1)!.count - historicalMonthly[0].count) / historicalMonthly[0].count * 100 : 0
  ...
  return <div className="app-shell unified-app">
    ...
    <main data-active-page={active}>
      ...
      <section id="overview" className="section-block">
        <div className="section-title"><span>공공데이터 기반 핵심 분석</span><h2>장기 기준선이 찾은 행정 검토 후보</h2><p>2020~2025년 민원 원자료로 규모·증가·반복·부서 부담을 분석합니다.</p></div>
        <div className="kpi-grid"><Kpi label="공공데이터 분석 행" value={`${number(data.bundle.publicComplaintRows)}행`} detail="두 민원 데이터셋의 원자료 행 합계" /><Kpi label="장기 분석 기간" value="2020~2025" detail="부분 집계 제외·동일기간 비교" /><Kpi label="검토 후보" value={`${data.baseline.priorities.length}개`} detail="자동 확정이 아닌 담당자 큐" /><Kpi label="2026 확인 레이어" value={`${number(official.detailRows)}건`} detail="별도 공개 범위·점수에 직접 합산 안 함" tone="current" /></div>
      ...
      <section id="trend" className="section-block"><div className="section-title"><span>공공데이터포털 기반</span><h2>장기 민원 추세를 연도·지역별로 비교</h2><p>그래프의 점에 마우스를 올리면 월별 정확한 값을 확인할 수 있습니다.</p></div><div className="trend-filter-bar"><div><span>연도</span>{['전체', '2020', '2021', '2022', '2023', '2024', '2025'].map((year) => <button className={trendYear === year ? 'active' : ''} key={year} onClick={() => setTrendYear(year)}>{year}</button>)}</div><div><span>지역</span>{['전체', '처인구', '기흥구', '수지구'].map((district) => <button className={trendDistrict === district ? 'active' : ''} key={district} onClick={() => setTrendDistrict(district)}>{district}</button>)}</div></div><div className="trend-summary"><Kpi label="선택 기간 합계" value={`${number(trendTotal)}건`} detail={`${trendDistrict} · ${trendYear}`} /><Kpi label="월평균" value={`${number(trendAverage)}건`} detail={`${historicalMonthly.length}개 월`} /><Kpi label="최고 월" value={`${number(trendPeak.count)}건`} detail={trendPeak.month} tone="current" /><Kpi label="기간 첫월 대비" value={`${trendChange >= 0 ? '+' : ''}${trendChange.toFixed(1)}%`} detail={`${historicalMonthly[0]?.month ?? '-'} → ${historicalMonthly.at(-1)?.month ?? '-'}`} /></div><article className="trend-visual"><div className="panel-heading"><h3>{trendDistrict} · {trendYear} 월별 민원량</h3><span><i /> 최고값 강조 · 단위 건</span></div><LineChart rows={historicalMonthly} label={`${trendDistrict} ${trendYear} 장기 민원 추세`} showLabels={historicalMonthly.length <= 12} /></article><div className="trend-table"><div className="trend-table-head"><b>월</b><b>민원량</b><b>월평균 대비</b><b>시각 비교</b></div>{historicalMonthly.map((row) => <div key={row.month}><strong>{row.month}</strong><b>{number(row.count)}건</b><em className={row.count >= trendAverage ? 'up' : 'down'}>{row.count >= trendAverage ? '▲' : '▼'} {Math.abs((row.count - trendAverage) / Math.max(trendAverage, 1) * 100).toFixed(1)}%</em><span><i style={{ width: `${row.count / Math.max(trendPeak.count, 1) * 100}%` }} /></span></div>)}</div></section>

      <section id="official2026" className="section-block"><div className="section-title"><span>용인시 공개 데이터 전용</span><h2>2026년 공개 민원에서 확인되는 것</h2><p>{data.officialComplaints.analysisScope}. 장기 점수와 혼합하지 않고 최신 확인등급으로만 사용합니다.</p></div><div className="kpi-grid"><Kpi label="2026년 5월 전체" value={`${number(official.latestTotal)}건`} detail={`전월 대비 ${official.latestRate > 0 ? '+' : ''}${official.latestRate.toFixed(1)}%`} tone="current" /><Kpi label="상세 공개 범위" value={`${number(official.detailRows)}건`} detail="전체 민원과 동일 모집단 아님" /><Kpi label="최다 공개 출처" value={data.officialComplaints.sourceDistribution[0].source} detail={`${number(data.officialComplaints.sourceDistribution[0].count)}건`} /><Kpi label="최다 반복 민원" value={data.officialComplaints.repeatedTitles[0].title} detail={`${number(data.officialComplaints.repeatedTitles[0].count)}건`} /></div><div className="official-only-grid"><article><h3>공식 분류</h3>{official.categories.map((row) => <p key={row.cvlcpt_clsf}><span>{row.cvlcpt_clsf}</span><b>{number(Number(row.tot_nocs))}건</b></p>)}</article><article><h3>상세 공개 출처</h3>{data.officialComplaints.sourceDistribution.map((row) => <p key={row.source}><span>{row.source}</span><b>{number(row.count)}건</b></p>)}<h3 className="subhead">반복 민원명 TOP 5</h3>{data.officialComplaints.repeatedTitles.slice(0, 5).map((row) => <p key={row.title}><span>{row.title}</span><b>{number(row.count)}건</b></p>)}</article></div><LineChart rows={currentMonthly} label="2026 용인시 공식 민원 추이" /><div className="latest-check-table"><div className="latest-check-head"><b>장기 이슈</b><b>2026 공개 상세 매칭</b><b>최신 확인</b><b>해석</b></div>{data.officialComplaints.latestChecks.map((row) => <div key={row.issue}><strong>{row.issue}</strong><span>{number(row.latestCount)}건 · {row.latestShare}%</span><em data-grade={row.latestGrade}>{row.latestGrade}</em><small>{row.latestCount ? '공개 범위에서 관련 문자열 확인' : '0건은 현재 문제 없음이 아닌 공개 범위 미확인'}</small></div>)}</div></section>
      ...
      <section id="priority" className="section-block"><div className="section-title"><span>통합 검토 큐</span><h2>장기점수와 최신 확인을 나란히 판단</h2><p>서로 다른 범위의 수치를 합산하지 않고 최종 검토상태만 규칙으로 제안합니다.</p></div><div className="decision-layout"><div className="decision-list">{data.baseline.priorities.map((row, index) => { const check = data.officialComplaints.latestChecks.find((item) => item.issue === row.issue); return <button key={row.issue} className={selected.issue === row.issue ? 'selected' : ''} onClick={() => setSelectedIssue(row.issue)}><b>{String(index + 1).padStart(2, '0')}</b><span><strong>{row.issue}</strong><small>장기 {row.score.toFixed(1)}점 · 최신 {check?.latestGrade} · {reviews[row.issue] ?? '미검토'}</small></span><em>{check?.reviewStatus}</em></button> })}</div><aside className="decision-detail"><span>담당자 판단</span><h3>{selected.issue}</h3><div className="decision-numbers four"><b>{selected.score.toFixed(1)}점<small>장기 기준점수</small></b><b>{selectedCheck.latestGrade}<small>2026 최신 확인</small></b><b>{number(selectedCheck.latestCount)}건<small>공개 상세 매칭</small></b><b>{selectedCheck.reviewStatus}<small>제안 상태</small></b></div><p>{selected.rationale}</p><div className="scope-warning"><CircleWarning />최신 0건은 문제 없음이 아니라 공개 상세 범위에서 확인되지 않았다는 의미입니다.</div><dl><dt>담당 후보</dt><dd>{selected.dominantDepartment}</dd><dt>시민 영향</dt><dd>{selected.impact}</dd><dt>판단 기간</dt><dd>{selected.comparisonPeriod}</dd></dl><div className="review-actions"><button onClick={() => review('승인')}>승인</button><button onClick={() => review('보류')}>보류</button><button onClick={() => review('추가확인')}>추가 확인</button></div></aside></div></section>

      <section id="report" className="section-block"><div className="section-title"><span>실행 보고</span><h2>선택 이슈 의사결정 메모</h2><p>장기 근거와 최신 확인등급, 담당자 판단을 한 파일로 내보냅니다.</p></div><div className="report-sheet"><span>Y:Q DECISION NOTE · {month(official.latestMonth)}</span><h3>{selected.issue}</h3><div className="kpi-grid"><Kpi label="장기 기준점수" value={`${selected.score.toFixed(1)}점`} detail="공공데이터포털 기반" /><Kpi label="2026 최신 확인" value={selectedCheck.latestGrade} detail={`${number(selectedCheck.latestCount)}건 매칭`} /><Kpi label="제안 상태" value={selectedCheck.reviewStatus} detail="두 점수를 직접 합산하지 않음" /><Kpi label="담당자 상태" value={reviews[selected.issue] ?? '미검토'} detail="최종 결정은 담당자" /></div><pre>{report}</pre><button className="official-refresh" onClick={saveReport}><Download /> 의사결정 메모 저장</button></div></section>
